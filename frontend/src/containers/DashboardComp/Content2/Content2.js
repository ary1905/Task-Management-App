...
export default function Content2() {
  ...
  const deleteTask = async (taskId) => {
    try {
      const token = localStorage.getItem('access');
      await axios.delete(`${process.env.REACT_APP_API_URL}/accounts/api/delete_task/${taskId}/`, {
        headers: {
          Authorization: `JWT ${token}`,
        },
      });
      setTasks((prevTasks) => prevTasks.filter((task) => task.task_id !== taskId));
      setCurrentSlide((prevSlide) => (prevSlide === tasks.length - 1 ? 0 : prevSlide));
    } catch (error) {
                  ...
                  <div className="flip-card-inner">
                    <div className="flip-card-front">
                      ...
                      <div className="frontBtns">
                        <button className="button1" onClick={() => deleteTask(tasks[currentSlide].task_id)}>
                        ...
                        <button className="button2" onClick={() => deleteTask(tasks[currentSlide].task_id)}>
